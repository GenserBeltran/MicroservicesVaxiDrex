...
export class UserEffects {
  ...
  signUpEmail: Observable<Action> = createEffect(() =>
    ...
    this.actions.pipe(
      ...
      switchMap(userData => //Ya puedo usar el map
        // this.httpClient.post<UserResponse>(`${environment.url}api/authentication/sign-up`, userData)//Se ejecuta si fue exitosa la Op adecuadamente
        this.httpClient.post<UserResponse>(`${environment.url}api/authentication/sign-up`, userData)//Se ejecuta si fue exitosa la Op adecuadamente
          ...
          .pipe(
            ...
            catchError(err => { //Este no crea un Observable
              this.notification.error("Register a new User Error");
  ...
  signInEmail: Observable<Action> = createEffect(() =>
    this.actions.pipe(
      ...
      switchMap(credentials => //Ya puedo usar el map
        this.httpClient.post<UserResponse>(`${environment.url}api/authentication/sign-in`, credentials)//Se ejecuta si fue exitosa la Op adecuadamente
          .pipe(
            tap((response: UserResponse) => { //Este pipe ecalua el posible resultado luego del llamado del la url Backend
              localStorage.setItem('token', response.token);
              this.router.navigate(['/']);
